...
the original example-alignment baseline. This checkout now runs those examples on Codex `0.157.1`.
...
No runtime or generated-protocol upgrade was needed to add ExternalMessage. The separate runtime
baseline upgrades track other upstream protocol changes, not a new requirement for this feature.
...
was initially verified on `0.153.4` and is reverified on the source checkout's `0.157.1`. Text and
