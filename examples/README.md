...
| Symptom | Recovery |
| --- | --- |
| npm returns 404 for the scoped package | Check `npm view @jaminzhou/codex-app-server-client@0.3.0 version --registry https://registry.npmjs.org/`. If unavailable, use a verified local candidate. Check network access; do not infer publication from a source version or substitute the unscoped package. |
