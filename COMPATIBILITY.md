...
| --- | --- | --- |
| Public Codex CLI | `codex-cli 0.147.0` / `rust-v0.147.0` | Runtime binary and public app-server behavior |
...
| --- | --- | --- |
| Typed client requests | Complete at the raw `call()` layer | All 136 generated client methods are mapped to generated parameter and response types |
...
| Runtime protocol validation | Complete for every generated request/notification/server-request shape and 133 of 136 client responses | Strict by default, including Rust signed/unsigned integer widths; three deprecated response types have no upstream JSON Schema |
...
`compatibility-matrix.json`, currently `0.146.1` and the pinned `0.147.0`. On the minimum supported
