...
| --- | --- | --- |
| Public Codex CLI | `codex-cli 0.150.1` / `rust-v0.150.1` | Runtime binary and public app-server behavior |
...
| --- | --- | --- |
| Typed client requests | Complete at the raw `call()` layer | All 156 generated client methods are mapped to generated parameter and response types |
...
| Runtime protocol validation | Complete for every generated request/notification/server-request shape and 153 of 156 client responses | Strict by default, including Rust signed/unsigned integer widths; three deprecated response types have no upstream JSON Schema |
...
`compatibility-matrix.json`, currently `0.149.1` and the pinned `0.150.1`. On the minimum supported
