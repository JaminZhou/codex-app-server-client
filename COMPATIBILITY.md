...
| --- | --- | --- |
| Public Codex CLI | `codex-cli 0.149.1` / `rust-v0.149.1` | Runtime binary and public app-server behavior |
...
| --- | --- | --- |
| Typed client requests | Complete at the raw `call()` layer | All 153 generated client methods are mapped to generated parameter and response types |
...
| Runtime protocol validation | Complete for every generated request/notification/server-request shape and 150 of 153 client responses | Strict by default, including Rust signed/unsigned integer widths; three deprecated response types have no upstream JSON Schema |
...
`compatibility-matrix.json`, currently `0.149.0` and the pinned `0.149.1`. On the minimum supported
